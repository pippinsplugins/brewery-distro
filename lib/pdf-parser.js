...
async function extractInvoiceData(pdfBuffer, accounts, inventory) {
  ...
  if (candidates.length > 0) {
    let bestMatch = { accountId: '', accountName: candidates[0], match: 'none' };
    ...
    // When no match, prefer the business name (last candidate) over contact name
    if (bestMatch.match === 'none' && candidates.length > 1) {
      bestMatch.accountName = candidates[candidates.length - 1];
    }
...
function extractABCLicense(text) {
  const m = text.match(/\bABC\s*#?\s*(\d{5,})/i);
  return m ? m[1] : '';
}
...
function extractAccountCandidates(text) {
  const lines = text.split('\n').map(l => l.trim()).filter(Boolean);
  const cleanABC = s => s.replace(/\s*-\s*ABC\s*#?\s*\d+.*$/i, '').trim();
  ...
  for (let i = 0; i < lines.length; i++) {
    if (/^customer/i.test(lines[i])) {
      ...
      for (let j = 1; j <= 3 && i + j < lines.length; j++) {
        ...
        if (/^\d/.test(line)) continue;                  // skip address
