...
async function extractInvoiceData(pdfBuffer, accounts, inventory) {
  ...
  // If no account name was found, try matching the contact name against accounts
  if (!parsed.accountName && parsed.contactName) {
    const contactMatch = matchAccount(parsed.contactName, accounts);
    if (contactMatch.match !== 'none') {
      parsed.accountId = contactMatch.accountId;
      parsed.accountName = contactMatch.accountName;
      parsed.accountMatch = contactMatch.match;
    } else {
  ...
  // Last resort: match by ABC license when name-based matching failed
  if (parsed.accountMatch === 'none' && parsed.abcLicense) {
    const abcMatch = accounts.find(a => a.ABCLicense && a.ABCLicense === parsed.abcLicense);
    if (abcMatch) {
      parsed.accountId = abcMatch.ID;
      parsed.accountName = abcMatch.Name;
      parsed.accountMatch = 'exact';
    }
  }
...
function matchProduct(productName, inventory, format) {
  ...
  if (fuzzy) {
    // Same format-mismatch guard as exact: don't match keg when invoice is a case
    if (fmtNorm) {
      const fuzzyFmtNorm = normalizeFormat(fuzzy.Format || '').toLowerCase().trim();
      if (fuzzyFmtNorm && fmtNorm !== fuzzyFmtNorm) {
        return { inventoryId: '', match: 'none' };
      }
    }
    return { inventoryId: fuzzy.ID, match: 'fuzzy' };
  }
  ...
  if (fmtLower) {
    ...
    if (fuzzyFmt) {
      if (fmtNorm) {
        const fuzzyFmtNorm2 = normalizeFormat(fuzzyFmt.Format || '').toLowerCase().trim();
        if (fuzzyFmtNorm2 && fmtNorm !== fuzzyFmtNorm2) {
          return { inventoryId: '', match: 'none' };
        }
      }
      return { inventoryId: fuzzyFmt.ID, match: 'fuzzy' };
    }
  }
