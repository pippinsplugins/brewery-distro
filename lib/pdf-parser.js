...
function extractAccountName(text) {
  ...
  // Strategy 2: Line-based — find a line containing "Customer" (often a table header in PDFs),
  // then take the NEXT line as the customer name (handles "CustomerInvoice DetailsPayment" → next line)
  for (let i = 0; i < lines.length; i++) {
    if (/^customer/i.test(lines[i]) && i + 1 < lines.length) {
      const nextLine = lines[i + 1];
      // Skip if next line looks like an email, address, or header
      if (nextLine && !/@/.test(nextLine) && !/^(item|product|description|#|page\s)/i.test(nextLine)) {
        // Clean up — strip trailing suffixes like " - ABC #12345"
        const cleaned = nextLine.replace(/\s*-\s*ABC\s*#\d+.*$/i, '').trim();
  ...
  if (broadMatch) {
    const name = broadMatch[1].trim().replace(/\s*-\s*ABC\s*#\d+.*$/i, '').trim();
...
function matchProduct(productName, inventory, format) {
  ...
  // Exact match on name + format combination (e.g. "Oktoberfest" matches Name, "16oz Can Case" matches Format)
  if (fmtLower) {
    const nameAndFormat = inventory.find(p => {
      const invName = (p.ProductName || p.Name || '').toLowerCase().trim();
      const invFormat = (p.Format || '').toLowerCase().trim();
      return invName === lower && invFormat && fmtLower.includes(invFormat.replace(/[()]/g, ''));
    });
