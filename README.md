...
- **Inventory** — Per-location stock levels with low-stock thresholds and alerts. Tracks allocated and available units against open orders; clicking the allocated count opens a list of the specific orders holding that stock. Stock is adjusted through stock movements (sales, received, write-offs, adjustments) for a full audit trail.
